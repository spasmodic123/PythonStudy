...
b = np.array([1.666, 2, 3, 4, 5], dtype='float',
             copy=False)  # 转化数据类型,copy默认为True,如果c=b操作,创建副本,对c的操作影响b; 如果False,不会创建副本,对c的操作也影响b
...
i = np.logspace(0, 5,6, base=3)  # 生成等比数数列,参数分别为 起始点:base的x次方,结束点:base的x次方,项数,比值
print(i)
...
print(l[[2, 3, -1]], end='\n\n')  # 二维数组索引,也要对应加一个中括号,对应第二行,第三行,最后一行
