...
'''#数据预处理, 数据归一化,原始数据经过数据归一化/标准化处理后，各指标处于同一数量级
        maximum = self.train_x.max(axis=0)  # axis=0代表竖轴,从左往右求每一列最大最小
        minimum = self.train_x.min(axis=0)
        average = self.train_x.sum(axis=0) / train_data.shape[0]
        ...
        # 归一化处理开始,归一化公式
        for i in range(13):
            self.train_x[:, i] = (self.train_x[:, i] - min[i]) / (max[i] - min[i])
        '''